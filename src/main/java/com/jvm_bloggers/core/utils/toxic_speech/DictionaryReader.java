...
import com.jvm_bloggers.entities.toxic_word.ToxicWord.SupportedLanguage;
...
interface DictionaryReader {
    ...
    static DictionaryReader englishReader(final String content) {
        return new EnglishDictionaryReader(
            new BufferedReader(
                new StringReader(content)));
    }

    static DictionaryReader polishReader(final String content) {
    ...
    class EnglishDictionaryReader implements DictionaryReader {
        ...
        @Override
        public Set<ToxicWord> read() {
            return Stream.ofAll(reader.lines())
                .map(String::strip)
                .filter(StringUtils::isNotBlank)
                .map(v -> new ToxicWord(SupportedLanguage.ENGLISH, v))
                .toSet();
        }
    }
    ...
    class PolishDictionaryReader implements DictionaryReader {
        ...
        public Set<ToxicWord> read() {
            return Stream.ofAll(reader.lines())
                ...
                .map(String::strip)
                .filter(StringUtils::isNotBlank)
                .map(v -> new ToxicWord(SupportedLanguage.POLISH, v))
                .toSet();
        }
    }
}
