...
class BlogPostRepositoryTextSearchSpec extends SpringContextAwareSpecification {
    ...
    def "should find approved blog posts containing given keyword in title"() {
        ...
        when:
        List<BlogPost> searchResult = repository.findApprovedPostsByTagOrTitle(keyword, PageRequest.of(0, 10))
        then:
    ...
    def "should count all approved blog posts containing given keyword in title"() {
        ...
        when:
        int kotlinBlogPostsCount = repository.countApprovedPostsByTagOrTitle(keyword)
        then:
    ...
    def "should count all approved blog posts by tag"() {
        ...
        when:
        int devopsBlogPostsCount = repository.countApprovedPostsByTagOrTitle(keyword)
        then:
    ...
    def "should return first page by tag or title and sorting by the published date desc"() {
        ...
        BlogPost transactionsInSpringPost = aBlogPost(title: "Transaction in Spring ", approved: true, blog: companyBlog,
                tags: [springTag], publishedDate: LocalDateTime.of(2021, 3, 1, 10, 0))
        ...
        when:
        List firstThreeTitles = repository.findApprovedPostsByTagOrTitle(keyword, PageRequest.of(0, 3))
                .collect { it.title }
        then:
